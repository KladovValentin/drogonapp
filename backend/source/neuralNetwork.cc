...
int NeuralNetwork::remakeInputDataset(bool draw){
    ...
    //vector< pair< int, vector<double> > > table = readClbTableFromFile(saveLocation+"info_tables/MDCModSecPreciseCosmic25_106VaryHVends9a.dat");
    vector< pair< int, vector<double> > > table = readClbTableFromFile(saveLocation+"info_tables/MDCModSecPreciseFeb24ends9a.dat");      //2024 features
    //vector< pair< int, vector<double> > > table = readClbTableFromFile(saveLocation+"info_tables/MDCModSecPreciseApr25ends9.dat");      //2025 features
    ...
    if (writeFile){
        ...
        for (auto entry: meanToTModSec){
            ...
            bool tooBadValues = false;
            for (size_t s = 0; s < 2*outShapeLength; s++){
                if (meanToTModSec[run][s] < 10 || meanToTModSec[run][s] > 200)
                    tooBadValues = true;
            }
            //if (tooBadValues)
            //    continue;
