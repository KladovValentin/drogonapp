    ...
    try {
        ...
        for (auto row = rows.begin(); row != rows.end(); ++row) {
            ...
            if (row["float_val"].is_null() || row["channel_id"].is_null() || row["smpl_time"].is_null()){
                if (row["float_val"].is_null())
                    cout << "float_val is null;   ";
                if (row["channel_id"].is_null())
                    cout << "channel_id is null;   ";
                if (row["smpl_time"].is_null())
                    cout << "smpl_time is null;   ";
                cout << endl;
                continue;
            }
            prevDate = dateValue;
            ...
            //if (dbId == 8670)
            //    cout << dateValue << "  |  " << dbId << "  |  " << dbValue << endl;
        ...
        cout << "Rows fetched: " << rowCount << endl;
        w.commit();
...
vector<double> EpicsDBManager::getDBdata(int run, int runnext){
    ...
    if (runnext-run > 600){
        int count = 0;
        for (int lo = run; lo < runnext; lo += 600) {
            count += 1;
            int ho = lo + 600 > runnext ? runnext : lo + 600;
            vector<double> resultL = getDBdata(runToDate(lo), runToDate(ho), runToDate(lo - 10000));
            if (result.size() == 0){
                for (size_t i = 0; i < resultL.size(); i++){
                    result.push_back(resultL[i]*(double)(ho-lo)/((double)(runnext-run)));
                }
            }
            else {
                for (size_t i = 0; i < resultL.size(); i++){
                    result[i] += resultL[i]*(double)(ho-lo)/((double)(runnext-run));
                }
            }
        }
    }
    else {
        result = getDBdata(runToDate(run), runToDate(runnext), runToDate(run - 100000));
    }
...
void EpicsDBManager::makeTableWithEpicsData(string mode, int runl, int runr){
    ...
    //const char* fileName = "MDCModSecPreciseApr25ends9";
    const char* fileName = "MDCModSecPreciseFeb24ends9a";
    //const char* fileName = "MDCModSecPreciseCosmic25_56VaryHVends9a";
    ...
    //vector<double> dbPars0 = getDBdata(509449272,509449363);

    size_t i = 0;
    while (i < runBorders.size()-1){
        //vector<double> dbPars = getDBdata(runBorders[i].first,runBorders[i].second);
        vector<double> dbPars = getDBdata(runBorders[i].first,runBorders[i+1].first);
...
/// @param mode new or app (manually check last run in case of app)509443403
